...
public:
  ...
  /** \brief Add an edge to the graph if it doesn't exist already.
     ...
     */
  inline bool addEdge(const EdgeT& edge);
  ...
  /** \brief Add an edge to the graph if it doesn't exist already.
     ...
     *  \return false if graph is not consistent
     */
  inline bool addEdge(const int vtx1_id, const int vtx2_id);
  ...
  /** \brief Clear graph and preallocate memory for vertices in the
     ...
     */
  inline void preallocateVertices(const int num_vertices);
  ...
  inline int getNumVertexNeighbors(const int vtx_id) const;
  ...
  /** \brief Get vertex at specified index.
     ...
     */
  inline bool getVertex(const int vtx_id, VertexT& vertex) const;
  ...
  /** \brief Get index of the neighbor vertex at a specified position in the
     ...
     */
  inline bool getVertexNeighbor(const int vtx_id, const int nbr_it, int& nbr_id) const;
  ...
  /** \brief Get indices of the neighbors of a vertex at specified index.
     ...
     *  \return FALSE if vertex with requested id doesn't exist in the graph.
     */
  inline bool getVertexNeighbors(const int vtx_id, std::vector<int>& neighbors) const;
  ...
  /** \brief Get index of an edge between specified vertices.
     ...
     */
  inline bool getEdgeId(const int vtx1_id, const int vtx2_id, int edge_id) const;
  ...
  /** \brief Get edge at specified index.
     ...
     */
  inline bool getEdge(const int edge_id, EdgeT& edge) const;
  ...
  /** \brief Get edge between specified vertices.
     ...
     *  \return false if edge does not exist
     */
  inline bool getEdge(const int vtx1_id, const int vtx2_id, EdgeT& edge) const;
  ...
  /** \brief Get edge vertex indices.
     ...
     *  \return FALSE if edge with requested id doesn't exist in the graph.
     */
  inline bool getEdgeVertexIds(const int edge_id, int& vtx1_id, int& vtx2_id) const;

  /** \brief Print the adjacency list of the graph. */
  inline void printAdjacencyList() const;

  /** \brief Print the edge list of the graph. */
  inline void printEdgeList() const;
...
protected:
  ...
  /** \brief Given a source vertex and a target vertex find the position of
     ...
     */
  inline int getVertexNeighborListPosition(const int src_vtx_id, const int tgt_vtx_id) const;
  ...
  /** \brief Given two vertices get their positions in each other's neighbor
     ...
     */
  inline bool getEdgeNeighborListPositions(const int vtx1_id, const int vtx2_id, int& vtx1_nbr_it,
