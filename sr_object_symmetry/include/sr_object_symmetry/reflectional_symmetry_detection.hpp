...
#ifndef SR_OBJECT_SYMMETRY_REFLECTIONAL_SYMMETRY_DETECTION_HPP
#define SR_OBJECT_SYMMETRY_REFLECTIONAL_SYMMETRY_DETECTION_HPP
  ...
  if (params_.rot_symmetries.size() > 0)  // rotational symmetriy exist so keep perpendicular ones only
  {
    ...
    for (size_t symId = 0; symId < symmetries_initial_.size(); symId++)
    {
      for (size_t rotId = 0; rotId < params_.rot_symmetries.size(); rotId++)
      {
        ...
        if (fabs(rot_symmetry.dot(symmetries_initial_[symId].getNormal())) > 0.985)
...
#endif  // SR_OBJECT_SYMMETRY_REFLECTIONAL_SYMMETRY_DETECTION_HPP
